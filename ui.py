...
def button(surface, pos_y, text=None, click_sound=None, pos_x = SCREEN_WIDTH//2 - BUTTONS_SIZES[0]//2, b_size = BUTTONS_SIZES):
    ...
    pygame.draw.rect(surface, COLORS["buttons"]["shadow"], (rect.x - 3, rect.y - 3, rect.w, rect.h),0,20) # draw the shadow rectangle
    pygame.draw.rect(surface, color, rect, 0, 20) # draw the rectangle
    # draw the text
    if text is not None:
